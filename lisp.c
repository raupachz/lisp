...
lval* lval_call(lenv* e, lval * f, lval* a) {
    ...
    while (a->count) {
        ...
        /* special case, varargs with ambersand */
        if (strcmp(sym->sym, "&") == 0) {
    ...
    /* argument list is now bound so we can clean up */
    lval_del(a);

    if (f->formals->count > 0 && strcmp(f->formals->cell[0]->sym, "&") == 0) {
        /* check to ensure ambersand is not passed invalidly */
    ...
    } else { /* return partial applied function */
        return lval_copy(f);
    }
}
...
lval* builtin_op(lenv* e, lval* a, char* op) {
    for (int i = 0; i < a->count; i++) {
        if (a->cell[i]->type != LVAL_NUM) {
            lval_del(a);
            return lval_err("Can not operate on a non-number");
        }
    }
...
lval* builtin_lambda(lenv* e, lval* a) {
    ...
    /* Pop first two arguments and pass them to lambda */
...
lval* builtin_var(lenv* e, lval* a, char* func) {
    ...
    /* First argument is a symnbol list */
    lval* syms = a->cell[0];
    /* Ensure all elements are symbols */
    for (int i = 0; i < syms->count; i++) {
        LASSERT(a, syms->cell[i]->type == LVAL_SYM,
            "Function '%s' cannot define non-symbol."
            "Got %s, exptected %s.", func,
    ...
    LASSERT(a, syms->count == a->count-1, 
        ...
        "Got %i, expected %s", func, syms->count, a->count - 1);
